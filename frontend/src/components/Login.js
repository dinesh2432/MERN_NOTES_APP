...
const Login = () => {
    ...
    const handleLogin = async(email,password)=>{
        try{
            const res = await axios.post(`https://mern-notes-app-86t8.vercel.app/api/auth/login`,{email,password},{withCredentials:true,headers:{"Content-Type":"application/json"}})
            if(res.status ==200){
                navigate('/notes')
            }
